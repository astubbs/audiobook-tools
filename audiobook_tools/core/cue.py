...
class CueProcessor:
    ...
    def get_audio_length(self, audio_file_path: Path) -> float:
        ...
        try:
            ...
            raise AudioFileError(f"Invalid ffprobe output for: {audio_file_path}") from e
    ...
    def parse_cue_file(self, cue_path: Path) -> CueSheet:
        ...
        try:
            ...
            raise InvalidCueFormatError(f"Invalid encoding in CUE file: {cue_path}") from e
        
        # Parse FILE directive
        file_match = next((line for line in lines if line.strip().startswith("FILE")), None)
        if not file_match:
        ...
        for line in lines:
            ...
            if line.startswith("TRACK"):
                ...
                current_track = Track(number=track_num, title=None, performer=None, index={})
            
            elif current_track:
                if line.startswith("TITLE"):
                    current_track.title = re.search(r'TITLE\s+"([^"]+)"', line).group(1)
                elif line.startswith("PERFORMER"):
                    current_track.performer = re.search(r'PERFORMER\s+"([^"]+)"', line).group(1)
    ...
    def combine_cue_sheets(self, cue_files: List[Path]) -> str:
        ...
        for i, cue_path in enumerate(cue_files):
            logger.info(f"Processing CUE file: {cue_path}")
            ...
            for track in cue_sheet.tracks:
                ...
                for index_num, timestamp in track.index.items():
                    ...
                    combined_content.append(f"    INDEX {index_num:02d} {adjusted_time}")
